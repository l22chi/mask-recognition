...
then
    ...
    echo "Whether or not to show extra visualizations for debugging ?"
    read EXTRA_VIZU
    echo -e "${YELLOW}[INFO] The exra visualizations you chose is : $EXTRA_VIZU${NC}"

    echo -e "${YELLOW}[INFO] executing script ...${NC}"

    python detect_with_classifier.py --image $IMAGE_NAME --size $ROI_SIZ --min-conf $MIN_PROB --visualize $EXTRA_VIZU
